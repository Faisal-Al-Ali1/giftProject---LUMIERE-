...
async function getPerfumeMenItems() {
  const dbRef = ref(db);
  try {
    const snapshot = await get(child(dbRef, "Products/natural"));
    if (snapshot.exists()) {
      ...
      // عرض كل منتج على DOM
      Object.entries(data).forEach(([key, naturalProducts]) => {
        const productCard = document.createElement("div");
        productCard.className = "productCard";

        productCard.innerHTML = `
            <div class="product-image">
                <img src="${naturalProducts.image}" alt="${naturalProducts.name}">
            </div>
            <p class="nameProduct">${naturalProducts.nameProduct}</p>
            <p class="detailsProduct">${naturalProducts.detailsProduct}</p>
            ...
            <p class="price">${naturalProducts.price}.00JD</p>
          <div class="button-container">
            <a href="details.html?category=natural&id=${key}" class="more-details">View More</a>
